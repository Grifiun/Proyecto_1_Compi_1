...
public class ControladorLogin extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String estadoLogin = funcionesLogin.verificarUsuarioLogin(usuario, password);
        String direccion;
        
        request.getSession().setAttribute("codigo", usuario);
        request.getSession().setAttribute("mensaje", "Bienvenido seas "+usuario);
        
        switch(estadoLogin){
            case "Usuario logueado con exito"://se loguea el usuario                
                direccion = "jsp/home-usuario.jsp";
                response.sendRedirect(direccion);
                break;
            default:
                request.getSession().setAttribute("mensajeError", estadoLogin);
